  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm">
         <div>
            <h3 className="text-2xl font-black text-gray-900 uppercase tracking-tight">My Wishlist</h3>
            <p className="text-sm text-gray-400 font-medium">Items you've saved for later.</p>
         </div>
         <div className="bg-gray-50 px-4 py-2 rounded-2xl flex items-center gap-2 text-orange-600 font-black text-sm">
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
           {[...Array(3)].map((_, i) => <div key={i} className="h-64 bg-gray-100 rounded-[2.5rem] animate-pulse" />)}
        </div>
      ) : wishlist.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
           {wishlist.map((item, idx) => (
             <motion.div 
               ...
               className="bg-white rounded-[2.5rem] border border-gray-100 p-4 hover:shadow-2xl transition-all group"
             >
                <div className="aspect-square bg-gray-50 rounded-[2rem] overflow-hidden relative mb-4">
                ...
                <div className="px-2">
                   ...
                   <p className="text-xl font-black text-orange-500 mt-1">₹{item.price?.toLocaleString()}</p>
                   <button className="w-full mt-4 py-3 bg-gray-900 text-white rounded-2xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-2 hover:bg-orange-500 transition-colors">
