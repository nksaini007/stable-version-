...
const CustomerConstruction = () => {
    ...
    const [activeTab, setActiveTab] = useState("STREAM"); // Mobile/Desktop Switch: NODES, STREAM, UTILS
    ...
    return (
        <div className="bg-[#e5e5e5] min-h-screen text-black font-mono selection:bg-[#ff5c00] selection:text-black overflow-hidden flex flex-col pt-20 tech-grid">
            ...
            <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-black border-t-4 border-[#ff5c00] flex">
                <button onClick={() => setActiveTab("NODES")} className={`flex-1 py-4 text-[10px] font-black uppercase tracking-widest ${activeTab === "NODES" ? 'bg-[#ff5c00] text-black' : 'text-white'}`}>NODES</button>
                <button onClick={() => setActiveTab("STREAM")} className={`flex-1 py-4 text-[10px] font-black uppercase tracking-widest ${activeTab === "STREAM" ? 'bg-[#ff5c00] text-black' : 'text-white'}`}>STREAM</button>
                <button onClick={() => setActiveTab("UTILS")} className={`flex-1 py-4 text-[10px] font-black uppercase tracking-widest ${activeTab === "UTILS" ? 'bg-[#ff5c00] text-black' : 'text-white'}`}>UTILS</button>
            </div>
                    ...
                    <>
                        ...
                        <div className={`md:col-span-2 border-r-2 border-black/5 flex flex-col h-full bg-white/30 backdrop-blur-md overflow-hidden ${activeTab !== "NODES" ? 'hidden md:flex' : 'flex'}`}>
                           <div className="p-8 border-b-2 border-black bg-black text-white">
                              ...
                              <h3 className="text-xl font-heading font-black tracking-tighter">PROJECTS</h3>
                           </div>
                           <div className="flex-1 overflow-y-auto scrollbar-tech p-4 py-8 space-y-4">
                              {projects.map((p, idx) => (
                                 ...
                                 >
                                    <div className="absolute top-2 right-2 text-[7px] font-black opacity-30">NODE_00{idx+1}</div>
                                    ...
                                    <span className="text-[7px] font-black opacity-40 uppercase mt-1 block tracking-wider">{p.location}</span>
                                 </div>
                              ))}
                           </div>
                        </div>

                        {/* ZONE 02: CENTRAL COMMAND STREAM (Split Independent Scrolls) */}
                        <div className={`md:col-span-10 lg:col-span-7 flex flex-col h-full overflow-hidden ${activeTab !== "STREAM" ? 'hidden md:flex' : 'flex'}`}>
                           {/* HEADER BAR (Fixed Area) */}
                           <div className="bg-white border-b-2 border-black flex flex-col p-6 px-10 gap-4 flex-shrink-0">
                                    ...
                                    <div className="flex justify-between items-start">
                                       <div>
                                          <div className="flex items-center gap-4 mb-2">
                                             ...
                                             <span className="text-[#ff5c00] font-black text-[9px] tracking-widest uppercase animate-pulse">● LIVE_NODE_ACTIVE</span>
                                          </div>
                                          <h1 className="text-3xl lg:text-5xl font-heading font-black leading-none uppercase tracking-tighter">{project.name}</h1>
                                       </div>
                           ...
                           <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
                              ...
                              <div className="lg:w-[40%] h-1/2 lg:h-full border-b-2 lg:border-b-0 lg:border-r-2 border-black/5 flex flex-col overflow-hidden">
                                 <div className="p-4 px-10 border-b border-black/5 bg-black/5 flex items-center justify-between">
                                    <span className="text-[10px] font-black uppercase tracking-[0.2em] opacity-40">SEC_01_LOG</span>
                                       ...
                                       <div key={project._id} className="space-y-6">
                                          {project.tasks.map((task, idx) => (
                                             <div key={task._id} className="bg-white border-2 border-black/5 p-6 relative group overflow-hidden shadow-sm hover:shadow-lg transition-all">
                                                ...
                                                <span className={`text-[8px] font-black px-2 py-0.5 border border-black uppercase mb-4 inline-block ${task.status === "Completed" ? 'bg-black text-white' : 'text-black'}`}>{task.status}</span>
                                                ...
                                                {task.images?.[0] && (
                                                   <div className="border border-black/10 aspect-video overflow-hidden mt-4">
                              ...
                              <div className="lg:w-[60%] h-1/2 lg:h-full flex flex-col overflow-hidden">
                                 <div className="p-4 px-10 border-b border-black/5 bg-black/5 flex items-center justify-between">
                                    <span className="text-[10px] font-black uppercase tracking-[0.2em] opacity-40">SEC_02_STREAM</span>
                                    ...
                                    {projects.filter(p => p._id === expandedProject).map(project => (
                                       <div key={project._id} className="space-y-12">
                                          {project.updates.length === 0 ? (
                                             <div className="text-center py-20 opacity-20"><FaCrosshairs size={40} className="mx-auto mb-4" /><span className="text-[10px] font-black">NO_STREAM_DATA</span></div>
                                          ) : (
                                             project.updates.map(upd => (
                                                <div key={upd._id} className="border-l-4 border-black p-8 relative bg-white/30 backdrop-blur-sm group">
                                                   <div className="flex items-center justify-between mb-4">
                                                      ...
                                                      <span className="text-[7px] font-black text-[#ff5c00] uppercase tracking-widest">LOG_ENTRY_SYNC</span>
                                                   </div>
                                                   ...
                                                   {upd.images?.[0] && (
                                                      <div className="border-4 border-black overflow-hidden aspect-video shadow-2xl relative">
                        ...
                        {/* ZONE 03: SYSTEM UTILITIES (Utility Panel) */}
                        <div className={`lg:col-span-3 border-l-2 border-black/5 flex flex-col h-full bg-white/30 backdrop-blur-md overflow-hidden ${activeTab !== "UTILS" ? 'hidden lg:flex' : 'flex'}`}>
                           <div className="p-8 border-b-2 border-black bg-white">
                              <div className="flex items-center gap-2 mb-2 opacity-40">
                                 ...
                                 <span className="text-[8px] font-black tracking-widest uppercase">UTL.RES_MATRIX</span>
                              </div>
                              <h3 className="text-xl font-heading font-black tracking-tighter">UTILITIES</h3>
                           </div>
                           
                           <div className="flex-1 overflow-y-auto scrollbar-tech p-8 space-y-12 pb-32 lg:pb-12">
                              {projects.filter(p => p._id === expandedProject).map(project => (
                                 <div key={project._id} className="space-y-10">
                                    <div className="space-y-8">
                                       <h4 className="text-[10px] font-black uppercase tracking-widest border-b-2 border-[#ff5c00] pb-2 inline-block">MATERIAL_INDEX</h4>
                                             ...
                                             return (
                                                <div key={m._id} className="space-y-2">
                                                   <div className="flex justify-between items-end">
                                                      <span className="text-[10px] font-black uppercase tracking-tight">{m.materialId?.name}</span>
                                                      <span className={`text-[10px] font-black ${pct > 90 ? 'text-[#ff5c00]' : 'text-black'}`}>{pct}%</span>
                                                   </div>
                                                   <div className="h-1 bg-black/10 overflow-hidden"><div className={`h-full ${pct > 90 ? 'bg-[#ff5c00]' : 'bg-black'}`} style={{ width: `${pct}%` }}></div></div>
                                                </div>
                                             );
                                          })}
                                       </div>
                                    ...
                                    <div className="bg-black p-8 text-white relative overflow-hidden group">
                                       <div className="absolute top-2 right-2 text-[#ff5c00]/40 text-4xl opacity-20"><FaShieldAlt /></div>
                                       <h4 className="text-[10px] font-black opacity-40 uppercase tracking-widest mb-6">SECURE_CONNECT</h4>
                                       <Link to="/support" className="block relative z-10"><button className="w-full py-5 bg-[#ff5c00] text-black font-black text-[10px] tracking-widest hover:bg-white uppercase flex items-center justify-center gap-2">CONNECT_ARCHITECT</button></Link>
                                    </div>
                                    
                                    <div className="border border-black/10 p-6 space-y-4 bg-white/50">
                                       <div className="flex items-center gap-2 text-black/40">
                                          <FaInfoCircle size={10} />
                                          <span className="text-[8px] font-black uppercase tracking-widest">SYSTEM_DIAGNOSTICS</span>
                                       </div>
                                       <div className="text-[9px] font-bold text-black opacity-50 uppercase leading-relaxed">Active stream monitoring successful. Signal latency &lt; 2ms. Project node ST-00{projects.indexOf(project)+1} initialized accurately.</div>
                                    </div>
                                 </div>
                              ))}
                           </div>
